...
class ChatBotModel(object):
    ...
    def _creat_optimizer(self):
        ...
        with tf.variable_scope('training') as scope:
            ...
            #                               2000, config.DECAY_FACTOR , staircase=True)

            self.learning_rate = tf.Variable(config.LR, dtype=tf.float32, trainable=False)

            self.learning_rate_decay_op = self.learning_rate.assign(
                                            self.learning_rate * config.DECAY_FACTOR)
            if not self.fw_only:
